...
> 기본 리더보드는 `--limit` 없이 전체 평가 세트를 끝까지 실행한 결과만 포함합니다. 기존 README 표에서 옮긴 행도 전체 평가 결과로 확인된 값만 `doc/submitted_results.json`에 curated full-evaluation row로 보관합니다. 재현 가능한 새 실행 아티팩트가 생기면 같은 스키마로 덮어써 출처를 갱신합니다.
...
- `--log_interval` (기본값: `1`)
- `--log_outliers` (이상치 샘플을 모두 로그에 출력)
...
- 고정 설정
  - Whisper: `whisper_tiny`, `whisper_base`, `whisper_small`, `whisper_medium` (`language="ko"`, `task="transcribe"`, deterministic decoding)
  - Qwen3-ASR: `qwen3_asr_0_6b`, `qwen3_asr_1_7b` (`language="Korean"` 고정)
- 동적 모델명 추론
...
- `doc/data/AIHubLowQualityTelephone.md`
- `doc/aihub_pipeline_check.md`
- `doc/README.md`
- `doc/data_policy.md`
